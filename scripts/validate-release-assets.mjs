...
const expectedTag = "v1.2.1";
const tag = process.argv.find((value) => value.startsWith("--tag="))?.slice(6) ?? process.env.GITHUB_REF_NAME;
if (expectedVersion !== "1.2.1") throw new Error(`Release validation requires package version 1.2.1, got ${expectedVersion}.`);
...
const staleArtifacts = readdirNames(releaseDir).filter(
  (name) => /^(?:UltraX-Browser-)?(?:Setup-)?\d+\.\d+\.\d+/.test(name) && !name.includes(expectedVersion),
);
